...
class UPDHandler(BaseDocumentHandler):
    ...
    DETECTION_PATTERNS = [
        ...
        r'университарный\s*передачный\s*документ', # OCR error variation
        r'\bупд\b',
        r'универсальный\s+документ\s+(?:о|об)\s+передаче',
        r'приложение\s*№\s*1\s*к\s*постановлению\s*правительства\s*рф\s*от\s*20\.12\.2011\s*№\s*1137', # UPD standard header
    ]
    ...
    REQUIRED_FIELDS = [
        ...
        },
        {
            ...
            'patterns': [
                ...
                r'(?:сделитель/дата|номер|no\.?)\s*(?:№|no\.?|#)?\s*[:\-]?\s*(\S+)',
                r'(?:номер|no\.?)\s*[:\-]?\s*(\S+)'
            ],
            'required': True,
            'region': 'header'
        ...
        },
        {
            'name': 'ИНН продавца',
            'patterns': [
                r'(?:инн|inn|inн|iнн|иhh|1nn|инкцип)\s*(?:продавца|поставщика)?\s*[:\-]?\s*(\d{10,12})',
                r'\b(\d{10,12})\b'
            ],
            'required': True,
            'region': 'provider'
        },
        {
            'name': 'КПП продавца',
            'patterns': [
                r'(?:кпп|инкцип)\s*(?:продавца|поставщика)?\s*[:\-]?\s*(?:\d{10,12}/)?(\d{9})',
                r'\b(\d{9})\b'
            ],
            'required': False,
            'region': 'provider'
        ...
        },
        {
            'name': 'ИНН покупателя',
            'patterns': [
                r'(?:инн|inn|inн|iнн|иhh|1nn|инкцип)\s*(?:покупателя|получателя)?\s*[:\-]?\s*(\d{10,12})',
                r'\b(\d{10,12})\b'
            ],
            'required': True,
            'region': 'customer'
        },
        {
            'name': 'КПП покупателя',
            'patterns': [
                r'(?:кпп|инкцип)\s*(?:покупателя|получателя)?\s*[:\-]?\s*(?:\d{10,12}/)?(\d{9})',
                r'\b(\d{9})\b'
            ],
            'required': False,
            'region': 'customer'
        ...
        },
        {
            'name': 'Всего к оплате',
            'patterns': [
                r'(?:^|\n)\s*(?:и\s*того|всего|итого|total|sum)\s*(?:к\s*оплате|х\s*оплаты)?\s*(?:\(\d+\))?[:\-—–\s]*([\d\s.,]+[.,]\d{2})\s*(?:руб|₽|rur)?',
