...
DOCUMENT_TYPES = {
    ...
    'Акт': {
        ...
        'fields': [
            ...
            {'name': 'ИНН исполнителя', 'patterns': [r'(?:инн|inn|inн|iнн|иhh|1nn)\s*(?:исполнителя)?\s*[:\-]?\s*(\d{10,12})', r'\b(\d{10,12})\b'], 'required': False},
            {'name': 'Заказчик', 'patterns': [r'заказчик\s*[:\-]?\s*(.+)', r'(?:заказ|customer)\s*[:\-]?\s*(.+)'], 'required': True},
            {'name': 'ИНН заказчика', 'patterns': [r'(?:инн|inn|inн|iнн|иhh|1nn)\s*(?:заказчика)?\s*[:\-]?\s*(\d{10,12})', r'\b(\d{10,12})\b'], 'required': False},
            {'name': 'Основание', 'patterns': [r'(?:основание|basis)\s*[:\-]?\s*(.+)', r'основание\s*[:\-]?\s*(.+)'], 'required': False},
            {'name': 'Итого', 'patterns': [r'(?:итого|всего|total|sum)\s*(?:к\s*оплате|по\s*акту)?\s*[:\-]?\s*([\d\s,]+(?:[.,]\d{2})?)'], 'required': True},
        ]
    },
    'Счет': {
        'patterns': [
        ...
        'fields': [
            ...
            {'name': 'Наименование получателя', 'patterns': [
                r'поставщик\s*[:\-]?\s*(?:["\']?)(ООО\s+"[^"]+"|АО\s+"[^"]+"|ПАО\s+"[^"]+"|ИП\s+[А-ЯЁ][а-яё]+\s+[А-ЯЁ][а-яё]+\s+[А-ЯЁ][а-яё]+)',
                r'поставщик\s*[:\-]?\s*(?:ooo|ооо|ао|пао|ип|ит)?\s*["\']?([\w\s"-]+?)(?:["\']?\s*,|\s*$|\s*инн)',
    ...
    for pattern in field_config['patterns']:
        ...
        if match:
            ...
            if value:
                ...
                if field_config['name'] in ['ИНН продавца', 'ИНН исполнителя', 'ИНН', 'БИК', 'Счет']:
                    # Clean for numeric fields
